...
from Peaks.ElutionModels import compute_moments, compute_egh_params
...
def recognize_peaks(x, y, num_peaks=5, exact_num_peaks=None, affine=False, model=None, min_area_prop=None, correct=True, debug=False):
    ...
    """
    from DataStructure.LPM import get_corrected
        ...
        if return_details:
            ...
            if baseline_type == 0:
                ...
                if baseline_type == 1:
                    ...
                    from Baseline.LinearBaseline import LinearBaseline as XrBaselineClass
                    from UV.UvPreRecog import UvPreRecog
                elif baseline_type == 2:
                    if debug:
                        from importlib import reload
                        import Baseline.IntegralBaseline
                        reload(Baseline.IntegralBaseline)            
                    from Baseline.IntegralBaseline import IntegralBaseline as XrBaselineClass
                elif baseline_type == 3:
                    if debug:
                        from importlib import reload
                        import Baseline.FoulingBaseline
                        reload(Baseline.FoulingBaseline)            
                    from Baseline.FoulingBaseline import FoulingBaseline as XrBaselineClass
                else:
                    assert False
