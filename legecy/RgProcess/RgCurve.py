...
import os
import numpy as np
from KekLib.SliceUtils import slice_consecutives
from _MOLASS.SerialSettings import get_setting
from Trimming import TrimmingInfo   # used in eval_file
...
def compute_rg_segment_wrapper(arg):
    from KekLib.SharedArrays import SharedArrays
...
def check_rg_folder(rg_folder):
    ...
    if ret_state:
        ...
        if os.path.exists(baseline_type_txt):
            from KekLib.EvalUtils import eval_file
...
class RgCurve:
    ...
    def proof_plot_impl(self, fig, axes):
        import KekLib.MatplotlibUtils      # required for annotate3D
