...
from SerialAnalyzer.DataUtils import serial_folder_walk, mct_folder_walk
from KekLib.TkTester import TestClient, messagebox_shown
from KekLib.MessageBoxUtils import reply_messagebox, window_exists
from KekLib.BasicUtils import clear_dirs_with_retry, Struct
from _MOLASS.SerialSettings import clear_settings, get_setting, set_setting, DF_PATH_LENGTH, DF_PATH_LENGTH_OLD
from KekLib.ExceptionTracebacker import ExceptionTracebacker
from Test.TesterLogger import *
...
class Tester:
    ...
    def test_all_entry( self, client, agent ):
        ...
        for i in self.test_patterns:
            ...
            if self.test_pattern == -1:
                ...
                from Test.TempInspection import inspection_close
        ...
        if self.shutdown_machine:
            from KekLib.Shutdown import shutdown_machine
            shutdown_machine()
    ...
    def test_a_folder( self, in_folder, uv_folder, plot=None, suppress_retry_call=False, analysis_name=None ):
        ...
        if not self.allow_mct_folder:
            from SerialAnalyzer.SerialDataUtils import get_mtd_filename
        ...
        if self.test_pattern == -1:
            if self.counter == 0:
                ...
                import Test.TempInspection
                reload(Test.TempInspection)
            from Test.TempInspection import datarange_problem_inspection
    ...
    def make_log_diff( self, app_logfile, from_logfile ):
        from KekLib.LogDiff import make_log_diff, time_re_sub
