...
import numpy as np
import logging
from UV.Absorbance import Absorbance
from . import FlowChange, TrimmingInfo, AutoRestrictor
from UV.PlainCurve import make_secondary_e_curve_at
from Mapping.CurveSimilarity import CurveSimilarity
from SerialAnalyzer.ElutionCurve import ElutionCurve
from Baseline.BaselineGuinier import change_baseline_correction_defaults
# from Trimming.PreliminaryRg import get_default_angle_range_impl
...
class PreliminaryRecognition:
    def __init__(self, sd, recog_only=False, debug=False):
        ...
        if recog_only:
            ...
            import Trimming
            reload(Trimming)
            from Trimming import AutoRestrictor
            pass
        else:
    ...
    def get_default_angle_range(self, sd):
        from DataStructure.LPM import get_corrected_matrix        # moved due to ImportError: cannot import name 'get_corrected_matrix' from partially initialized module 'LPM' (most likely due to a circular import)
    ...
    def set_restrict_info(self, sd, debug=False, figfile=None):
        ...
        if use_moment_trimming:
            from Trimming.MomentTrimming import get_moment_trimming_info
            from Trimming.TrimmingUtils import get_default_uv_wl_trimming
            from Trimming.AutoRestrictorProxy import AutoRestrictorProxy
