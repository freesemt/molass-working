...
from _MOLASS.SerialSettings import get_setting, setting_file, save_settings
from Models.ElutionCurveModels import EGH, EGHA
from Models.ModelUtils import compute_area_props
...
class LrfSource:
    ...
    def get_egh_moments_list(self):
        from Models.Stochastic.MomentUtils import compute_egh_moments
    ...
    def guess_monopore_params(self, old_style_return=False, debug=False):
        ...
        reload(module)
        from Models.Stochastic.MonoporeMoments import study_monopore_moments_impl
    ...
    def guess_lnpore_params(self, return_rgs=False, use_study=True, use_moments=True, progress_cb=None, debug=False):
        if use_study:
            ...
            from Models.Stochastic.MomentsStudy import moments_study_impl
            ...
            from Models.Stochastic.MomentUtils import compute_egh_moments
            from Models.Stochastic.MonoporeGuess import guess_monopore_params_using_moments
            from Models.Stochastic.LognormalGuess import guess_lognormalpore_params_using_moments
            ...
            if use_moments:
                from Models.Stochastic.RoughGuess import guess_monopore_params_roughtly
                ...
                from Models.Stochastic.SecModelUtils import guess_monopore_params_from_rgdist
    ...
    def moments_demo(self, adapt_backward=True, debug=True):
        if debug:
             import Models.Stochastic.MomentUtils
             reload(Models.Stochastic.MomentUtils)
        from Models.Stochastic.MomentUtils import moments_demo_from_rgdist
        rgs, trs, orig_props, peak_rgs, peak_trs, props, indeces, qualities = self.compute_rgs(debug=False)
    ...
    def monopore_study(self, debug=True):
        ...
        from Models.Stochastic.MonoporeStudy import study
        rgs, trs, orig_props, peak_rgs, peak_trs, props, indeces, qualities = self.compute_rgs(debug=False)
    ...
    def lnpore_study(self, debug=True):
        ...
        from Models.Stochastic.LnporeStudy import study
        rgs, trs, orig_props, peak_rgs, peak_trs, props, indeces, qualities = self.compute_rgs(debug=False)
        study(self.xr_x, self.xr_y, self.baselines[1], self.model, self.xr_peaks[indeces,:], peak_rgs, props)

    def run_simulation_process(self, parent, debug=True):
        import multiprocessing as mp
        from _MOLASS.Processes import register_process
        if debug:
