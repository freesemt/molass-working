...
from Decomposer.FitRecord import MIN_SEPARATE_AREA_RATIO, IGNORABLE_AREA_RATIO
...
def model_debug_plot(model, params, x_, y_, x, y, tau_hint, where_text, residual, before_params=None, print_callstack=False, work_y=None):
    ...
    if print_callstack:
        from KekLib.CallStack import CallStack
        print('model_debug_plot', CallStack())

    from KekLib.OurMatplotlib import get_color
    plt.push()
...
def plot_component_curves(ax, x, y, params_list, baseline, affine=False, color='blue', add_moments=False):
    from Models.EGH import egh, egha    # placed here to avoid circular import
        ...
        try:
            ...
            if debug:
                def get_range_except():
                    ...
                    from Models.ModelUtilsException import get_range_exceptionally
